...
import { Context as WomenContext } from '../context/dataContext'
import ProductDescription from '../components/ProductDescription'
import Filter from '../components/Filter'

const Women = ({ navigation }) => {
  const { state } = useContext(WomenContext)
  ...
  return (
    <View>
      ...
      {filter ? (
        <Filter filter={filter} close={filt} prod={state.womenProducts} />
      ) : null}
    </View>
  )
}
...
export default withNavigation(Women)